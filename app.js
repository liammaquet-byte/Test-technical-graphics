...
const state = {
  mode:'two', sp:{x:-4,y:3.2,z:-7}, pp:0, obj:{x:0,y:0,z:5,rot:28},
...
function objectPoints(){
 const w=2.4,d=2.0,h=2.0, r=modeRot()*Math.PI/180, cs=Math.cos(r), sn=Math.sin(r);
...
function modeRot(){return state.mode==='one'?0:state.obj.rot;}
function applyThreePoint(pr,w,h){
  if(state.mode!=='three') return pr;
  const vp3={x:w/2+95,y:-220};
  return pr.map(p=>{const k=Math.max(0,Math.min(.18,(p.y-h*.25)/h));return {x:p.x+(vp3.x-p.x)*k,y:p.y};});
}
function drawPerspective(){
 ...
 drawObj(ctx,pr); if(state.show.curves)drawCurves(ctx,pr);
...
function drawStep(ctx,w,h){
 ...
 $('feedback').textContent=messages[state.mode];
}
